...
1. **variable セクションを定義**
    ...
    <Table
        ...
        data={[
            ...
            ["type", ["変数の型を指定する、string など"].join("<br/>")],
            ["default", ["値が設定されなかった場合の既定値"].join("<br/>")],
        ]}
    />

    :::caution[variable セクションを参照する]
    xxx .tf というファイルがあった場合、その中で変数を参照している場合、同階層の xxx.tf に記述されているものを参照する。
    ...
    00_vpc.tf が参照するファイルは 00_subnet.tf、99_variable.tf です。そのため、00_vpc.tf 内で変数を参照するときに、二つのファイルに宣言がなければ参照エラーとなる。
